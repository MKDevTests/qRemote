...
ensure_prebuild() {
    ...
    if [[ $needs_prebuild == 1 ]]; then
        # Stop the Gradle daemon first. On Windows it keeps open handles on
        # android/app/build/**, and `prebuild --clean` then dies half-way with
        # "EBUSY: resource busy or locked, unlink … classes.dex", leaving a
        # gutted android/ behind. Harmless everywhere else.
        if [[ -x "$repo_root/android/gradlew" ]]; then
            (cd "$repo_root/android" && ./gradlew --stop >/dev/null 2>&1) || true
        fi
        echo "==> expo prebuild -p android"
        (cd "$repo_root" && npx expo prebuild -p android --no-install --clean)
    else
